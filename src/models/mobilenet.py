"""
MobileNetV2 Baseline Model Architecture for Solar Panel Fault Detection.

Features:
- ImageNet-pretrained MobileNetV2 backbone (torchvision)
- Customized 6-class classification head with dropout
- Model parameter inspection and memory footprint estimation
- Benchmark utility for inference latency and throughput (FPS)
"""
...
logger = setup_logger("mobilenet_model")
...
def build_mobilenet_v2(
    ...
    """
    Constructs a MobileNetV2 model adapted for 6-class solar panel defect classification.
    ...
    Returns:
        nn.Module: Configured PyTorch MobileNetV2 model.
    """
    weights = MobileNet_V2_Weights.DEFAULT if pretrained else None
    model = mobilenet_v2(weights=weights)

    # In MobileNetV2, classifier is:
    # Sequential(
    ...
    logger.info(
        f"Built MobileNetV2 (pretrained={pretrained}, num_classes={num_classes}, dropout={dropout})"
    )
    return model
...
def benchmark_inference_latency(
    ...
    """
    Measures reproducible per-image inference latency and frames per second (FPS).
    """
