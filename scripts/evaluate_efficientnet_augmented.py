"""
CLI Script to evaluate the best Augmented EfficientNet-B0 checkpoint on the independent test set.
Usage:
    python scripts/evaluate_efficientnet_augmented.py [--checkpoint PATH]
"""
...
logger = setup_logger("evaluate_efficientnet_augmented_cli")
...
def evaluate_checkpoint(checkpoint_path: str | Path | None = None):
    ...
    if checkpoint_path is None:
        checkpoint_file = root / "models" / "checkpoints" / "efficientnet_b0_augmented_best.pth"
    else:
    ...
    # Load test set (177 images, strictly deterministic preprocessing)
    ...
    acc_diff = (metrics["accuracy"] - effnet_base_acc) * 100.0
    f1_diff = (metrics["macro_f1"] - effnet_base_f1) * 100.0
    ...
    metrics["baseline_comparison"] = {
        ...
        "accuracy_improvement_pct_pts": round(acc_diff, 2),
        "macro_f1_improvement_pct_pts": round(f1_diff, 2),
    }
    ...
    plot_confusion_matrix(
        ...
        class_names,
        cm_path,
        title="EfficientNet-B0 + Augmentation — Confusion Matrix (Test Set)",
    )
    ...
    if cm_path.exists():
        shutil.copy2(cm_path, brain_plot_dir / cm_path.name)

    # Print Formatted Report
    print("\n" + "=" * 75)
    print(" EFFICIENTNET-B0 + AUGMENTATION EVALUATION REPORT")
    print("=" * 75)
    ...
    print("-" * 75)
    print(f"Test Accuracy          : {metrics['accuracy'] * 100:.2f}% (Baseline: 85.31% -> Diff: {acc_diff:+.2f}%)")
    ...
    print(f"Macro F1 Score         : {metrics['macro_f1'] * 100:.2f}% (Baseline: 84.93% -> Diff: {f1_diff:+.2f}%)")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Augmented EfficientNet-B0 checkpoint.")
